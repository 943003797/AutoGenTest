...
async def main() -> None:
    ...
    tools_filesystem = await mcp_server_tools(StdioServerParams(command="npx.cmd", args=["-y", "@modelcontextprotocol/server-filesystem", "S:\\"]))

    fetch_mcp_server = await mcp_server_tools(StdioServerParams(command="python", args=["mcp/mcp_server_fetch.py"]))

    tools_tivily = await mcp_server_tools(StdioServerParams(
        command= "node",
        args= ["C:\\Users\\94300\\Documents\\Tavily-MCP\\build\\index.js"],
        env= {
            "TAVILY_API_KEY": "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"
        }
        )
    )
    ...
    agent_search = AssistantAgent(
        ...
        system_message="你是古诗词大师，会根据用户提示，列举诗句。",
    ...
    stream = reflection_team.run_stream(task="分析https://www.ithome.com/0/839/110.htm")
